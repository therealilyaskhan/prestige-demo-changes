...
var HeightObserver = class extends HTMLElement {
  ...
  connectedCallback() {
    if (!window.ResizeObserver) {
      document.documentElement.style.setProperty(`--${this.getAttribute("variable")}-height`, `${Math.round(this.clientHeight) - 1}px`);
    }
  }
};
...
updateCustomProperties_fn = function(entries) {
  entries.forEach((entry) => {
    if (entry.target === this) {
      ...
      document.documentElement.style.setProperty(`--${this.getAttribute("variable")}-height`, `${Math.round(height) - 1}px`);
    }
  });
};
...
onVariantChanged_fn5 = function(event) {
  ...
  // mik added code for inventory indicator block on product page
  let stockIndicator = document.getElementById("stock-indicator");
  
  if (stockIndicator) {
    let stockTextualDesc = stockIndicator.querySelector(".icon-and-text__text");
    let threshold = parseInt(stockIndicator.dataset.threshold);
    let inStockText = stockIndicator.dataset.inStockText;
    let lowStockText = stockIndicator.dataset.lowStockText;
    let noStockText = stockIndicator.dataset.noStockText;
    let variantInventoryData = document
      .querySelector(".js-product-inventory-data")
      .querySelector(`[data-id='${variant.id}']`);
    let quantity = parseInt(variantInventoryData.dataset.quantity);
    let policy = variantInventoryData.dataset.policy;
  
    if (variant.inventory_management == "shopify") {
      if (policy == "continue") {
        //instock
        stockIndicator.classList.remove("inventory--low", "inventory--zero");
        stockTextualDesc.textContent = inStockText;
        ...
        //low stock
        stockIndicator.classList.remove("inventory--zero");
        stockIndicator.classList.add("inventory--low");
        stockTextualDesc.textContent = lowStockText;
      } else if (quantity <= threshold && quantity <= 0) {
        //no stock
        stockIndicator.classList.remove("inventory--low");
        stockIndicator.classList.add("inventory--zero");
        stockTextualDesc.textContent = noStockText;
      }
    }
  }
  // mik added code for inventory indicator block on product page
  ...
  // mik added
  let klarnaModalPrice = document.getElementById('klarna-modal-price');
  if(klarnaModalPrice){
    klarnaModalPrice.textContent = formatMoney(variant["price"], currencyFormat);
  }
  document.querySelectorAll(".paypal-placement").forEach((el) => {
    el.dataset.ppAmount = variant["price"];
  });
};
...
var ProductList = class extends HTMLElement {
  ...
  reveal() {
    ...
    observer.observe(document.querySelector('product-list'), { attributes: true, subtree: false, childList: true });
    // mik added custom javascript for smooth animation when new childnodes are added to reveal-items elements
